...
void processData(void)
{
  if(gotRxStruct)
  {
    ...
    case 3: 
      getDHT11();
      sendStruct(rxStruct.fromNodeID, 10, temperature);
      break;

    case 4:
      getDHT11();
      sendStruct(rxStruct.fromNodeID, 11, humidity);
      break;
...
void periodicallySendTempHum(void)
{
  if(millis() > nextSend)
  {
    if(!temperatureSent)
    {
      getDHT11();
      sendStruct(1, 10, temperature);
      temperatureSent = true;
      nextSend = millis() + delayTime;
    }
    else
    {
      getDHT11();
      sendStruct(1, 11, humidity);
